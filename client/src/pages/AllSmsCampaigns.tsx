...
import { MessageSquare, Calendar, Users, CheckCircle, XCircle } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
export default function AllSmsCampaigns() {
  const { data: campaigns, isLoading } = trpc.dealers.allCampaigns.useQuery();

  const smsCampaigns = campaigns?.sms || [];
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Tüm SMS Kampanyaları</h1>
          <p className="text-gray-500 mt-1">
            Tüm bayilerin oluşturduğu SMS kampanyalarını görüntüleyin
          </p>
        </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-600">
                    {smsCampaigns.filter((c) => c.status === "completed").length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-yellow-600">
                    {smsCampaigns.filter((c) => c.status === "sending").length}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-red-600">
                    {smsCampaigns.filter((c) => c.status === "failed").length}
                  </p>
                </div>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-500">
                  Henüz hiç SMS kampanyası oluşturulmamış
                </p>
              </div>
            )}
          </CardContent>
