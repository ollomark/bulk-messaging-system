...
function looksLikeRadio(url) {
  return (
    /somafm|icecast|radio|stream|\.mp3(\?|$)/i.test(url) && !/youtube|youtu\.be|soundcloud/i.test(url)
  );
}
...
export async function resolveQuery(query) {
  ...
  if (isHttpUrl(q) && looksLikeRadio(q)) {
    return { url: q, title: q, loop: true, mode: "direct" };
  }

  // Direct http that isn't youtube — try as stream file
  if (isHttpUrl(q) && !/youtube|youtu\.be|youtu\.be|soundcloud|spotify/i.test(q)) {
  ...
  if (!entry?.webpage_url && !entry?.url && !entry?.id) {
